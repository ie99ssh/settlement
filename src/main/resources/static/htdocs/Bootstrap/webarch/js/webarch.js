...
(function($) {
    ...
    Webarch.prototype.initPageControl = function(el){
    	try {
            $(".select2").select2();
        } catch(ex) { }
        
        try {
        	$("[class*=divideline]").each(function(){
        		$(this).prop("style", "border-top:1px dashed #c2c2c2; margin:5px 0 8px 0;");
        	});
        } catch(ex) { }
        
        $("body .table thead tr th").attr("style", "font-weight:bold; border:1px solid #ddd; background-color:#ecf0f2; text-align:center;");
        $("body .table tbody tr td").attr("style", "font-weight:bold; border:1px solid #ddd;");

        $('.grid .tools .collapse, .grid .tools .expand, .grid .tools .remove').on('click', function () {
            var el;        
            
            if (jQuery(this).hasClass("collapse") || jQuery(this).hasClass("remove")) {
                el = jQuery(this).parents(".grid").children(".grid-body");
                
                fnClearFormData();

                if (jQuery(this).hasClass("remove")) {
                    jQuery(this).parent(".grid .tools .expand").removeClass("collapse").addClass("collapse");
                    el.slideUp(200);
                    $("#spn_frm_title").text("등록");
                    $("#div_editFrm").hide(200);
                } else {
                    jQuery(this).removeClass("collapse").addClass("expand");
                    el.slideUp(200);
                }
            } else {
                el = jQuery(this).parents(".grid").children(".grid-body").not($("[id^='div_']"));
...
var fnSetEditFrm = function(editType) {
	el = jQuery("#div_editFrm .grid .tools").parents(".grid").children(".grid-body").not($("[id^='div_']"));
    jQuery(this).removeClass("expand").addClass("collapse");
    fnClearFormData();
    el.slideUp(200).slideDown(200);
    
    $("#spn_frm_title").text(editType == "regist" ? "등록" : "수정");
    $("#div_editFrm").show(200);
}

var fnSetEditFrmEx = function(divID) {
	el = jQuery(divID + " .grid .tools").parents(".grid").children(".grid-body").not($("[id^='div_']"));
    jQuery(this).removeClass("expand").addClass("collapse");
    fnClearFormDataEx(divID);
    el.slideUp(200).slideDown(200);
...
var fnClearValidator = function (divID) {
    var icon   = $(divID).find('.input-with-icon').children('i');
    var parent = $(divID).find('.input-with-icon');
...
var fnClearFormDataEx = function (divID) {
    fnClearValidator(divID);

    $(divID).find("input, select, checkbox, radio, textarea").each(function () {
        var nodeName = $(this)[0].nodeName;
